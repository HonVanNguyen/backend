...
export function IsValidEnum<E extends object, T>(
  ...
  return function (target: any, propertyKey: string | symbol): void {
    const { required = true, message = [propertyKey, 'common.invalid'] } = opts;
...
export function IsValidDate(
  ...
  return function (target: any, propertyKey: string): void {
    message = message || ([propertyKey, 'common.invalid'] as any);
...
export function IsValidText(
  ...
  return function (target: any, propertyKey: string): void {
    message =
      message || ([`exception.${propertyKey}`, 'common.invalid'] as any);
      ...
      validator: {
        ...
        defaultMessage(args: ValidationArguments) {
          const message =
            ...
            ([propertyName, 'common.invalid'] as any);
...
export function IsValidEnumNumber(
  ...
  return function (target: any, propertyKey: string | symbol): void {
    const message = opts?.message || ([propertyKey, 'common.invalid'] as any);
...
type ValidationJSONOptions = {
  required?: boolean;
} & I18nMessage;

export function IsValidJSON(
  { required = true, message }: ValidationJSONOptions = {
    required: true,
  },
): PropertyDecorator {
  return function (target: any, propertyKey: string | symbol): void {
    ...
    IsObject({ message: JSON.stringify(message) })(target, propertyKey);
    if (required)
      IsDefined({ message: JSON.stringify(message) })(target, propertyKey);
    else IsOptional()(target, propertyKey);
  };
